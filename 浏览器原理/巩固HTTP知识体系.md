> 文章来源：[掘金作者 - TianTianUp](https://juejin.im/post/6857287743966281736)
...
> 感谢掘金大佬 TianTianUp 提供的文章，也谢谢大家的支持🌹

> 本文囊括了HTTP大部分高频考点，让我们一起来掌握以下。
...
4. **无状态**，指服务端无法对同一客户端的多次http请求进行跟踪。
...
- 引入了**持久连接**，即TCP连接默认**不关闭**，可以被多个请求**复用**，不用声明Connection: **keep-alive**。长连接的连接时长可以通过请求头中的 **keep-alive** 来设置
- 引入了**管道机制**，即在同一个TCP连接里，客户端可以同时发送**多个请求**，进一步改进了HTTP协议的效率。
...
- HTTP/1.1 版本默认允许**复用TCP连接**，但是在同一个TCP连接里，所有数据通信是按次序进行的，服务器通常在处理完一个回应后，才会继续去处理下一个，这样子就会造成**队头阻塞**。
...
![](https://tva1.sinaimg.cn/large/007S8ZIlly1ghx59jr2z2j30z60gowi4.jpg)
...
![DNS工作流程](https://tva1.sinaimg.cn/large/007S8ZIlly1ghx5g58f0yj30zk06mwf5.jpg)

这张图很生动的展示了DNS在本地DNS服务器是如何查询的，**一般向本地DNS服务器发送请求是递归查询的**

本地 DNS 服务器向其他域名服务器请求的过程是迭代查询的过程👇

![本地DNS查询](https://tva1.sinaimg.cn/large/007S8ZIlly1ghx5ha1shcj30xm0jsta0.jpg)
...
> 能在多次HTTP之前重用同一个TCP连接，从而减少创建/关闭多个 TCP 连接的开销（包括响应时间、CPU 资源、减少拥堵等）

参考如下示意图（来源：维基百科）：![](https://tva1.sinaimg.cn/large/007S8ZIlly1ghx5j55p6sj30fb096aam.jpg)

### 客户端如何开启

> 在HTTP/1.0协议中，默认是`关闭`的，需要在http头加入**"Connection: Keep-Alive"**，才能启用**Keep-Alive**；

```js
Connection: keep-alive
```

> **http 1.1**中`默认启用Keep-Alive`，如果加入**"Connection: close"** ，才关闭。
...
- 精度上，**ETag**优于**Last-Modified**。ETag按照内容给资源带上标识，能准确感知资源变化，Last-Modified在某些场景并不能准确感知变化，比如👇
...
```
这个URL中，https就是协议，[www.baidu.com就是域名，默认端口是443，/s就是请求的path，](http://www.baidu.xn--com%2C443%2C-m30n8mr27ag5meu5aea2970fg4vc0hxd/s就是请求的path，)**tn=baidu&bar=&wd=TianTian**这个就是query。
...
![HTTP与HTTPS区别](https://tva1.sinaimg.cn/large/007S8ZIlly1ghx662uzo7j30uo0e20tk.jpg)
...
![中间人获取公钥](https://tva1.sinaimg.cn/large/007S8ZIlly1ghx6b5vvhnj30kl0n6q5t.jpg)
...
![](https://tva1.sinaimg.cn/large/007S8ZIlly1ghx6bqutpnj30fg0eljsk.jpg)
...
![](https://tva1.sinaimg.cn/large/007S8ZIlly1ghx6chjka1j30jx0e5tao.jpg)
